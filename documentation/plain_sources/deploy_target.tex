...
After building all relevant images, in the next step we must bring in the new
software into our target.
...
To bring in the new software we start to bring in the new bootlader. This is
the trickiest part, as we need special tools on our host and the target and
we have to deal with confusing error messages.

First of all, we must change the \textbf{S2} switch on our Mini2440
into the \texttt{NOR} position to start the internal \textit{vivi} bootloader.
...
We want to use the \textit{vivi} bootloader to bring in the new barebox bootloader
into Mini2440's RAM. In order to do so, we need the size in bytes of
barebox's binary:
...
With this size we instruct the \textit{vivi} bootloader to expect this amount of
...
Please consider the \textit{147844} number here. This number must be adapted to
your own size of barebox's image size.
...
If the host was able to enumerate the Mini2440 successfully can be
checked with the \texttt{lsusb} command. If the following line occurs in the
list, the Mini2440 is successfully enumerated:
...
When the transfer was successfull, the \texttt{usbpush} host tool will output:
...
After the transfer was successfull, we can now run the downloaded bootloader:
...
This will start the barebox bootloader on the Mini2440 that will
greet us with:
...
We can stop the autoboot timeout by hitting any key. We are now in the shell
...
If you do not see reasonable values here and you are using the DHCP option, run
...
Edit the lines beginning with \texttt{eth0.*} and give them reasonable values.
...
Running the \texttt{devinfo eth0} command again should now show reasonable
values for the network interface. If it really works can be tested by pinging
other hosts:
...
In order to store all relevant components into the NAND, we can use some builtin
features of barebox right now.
...
now back into the \texttt{NAND} position. Powercycle the Mini2440
...
%   sucessfull)
% - Add info how to check the downloaded file if it is the correct one
%
