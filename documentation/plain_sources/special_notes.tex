...
The predifined Mini2440 platform configuration always uses the most
available Linux kernel release. But if users are sticking to an older Linux
kernel release, also the older releases are still available. Here is a list
of currently available Linux kernel releases in the \ptxdistBSPName{}:
...
If another release than the default one should be used to build this board
support package, we just have to run \texttt{ptxdist platfomconfig} and
change the kernel setting prior the build.
...
Depending on the framebuffer device different kernel command line are mostly
needed to do this. Please refer to the manual of your display driver for more
details.
...
entries are appearing in sysfs. Everything is controlled via read- and writable
files to generate events on the digital lines.

We find all control files in \texttt{/sys/class/gpio}. In that path, there are
a number of \texttt{gpiochipXXX} entries, with \texttt{XXX} being a decimal
number. Each of those folders provides information about a single gpio
controller registerd on the Mini2440 board, for example with
...
\texttt{ngpio} contains the whole amount of GPIOs provided by this GPIO
controller.\\
We use \texttt{GPIO\perGpioNumber{}} as an example to show the usage of single
GPIOs.
...
necessary to write a kernel driver. The shown method works fine for example to
...
so-called triggers, which trigger the LED according to f.e. a simple timer.
...
unplugg the device while its still mounted. This may result in data loss.
\end{important}
...
device node, MMC/SD card partitions will occure in the following way:
...
Note: These partition device nodes will only occure if the card contains a
...
driver if it provides access to the SPI slave device through this way (sysfs),
...
% - how to use the Minit2440 as a gadget (printer, serial or ...)
...
Information and the latest release of the Mini2440 BSP you can find
at our website at
...
Mini2440 you can use the git repository as your working source
instead of a release archive.
